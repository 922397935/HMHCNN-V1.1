...
from models.decoders import Line_conv_1, Derivate_conv_1
...
class MHCNN_L(Encoder):
    """
    Hypergraph and Line Graph dual-channel convolution network.
    """

    def __init__(self, c, args):
        super(MHCNN_L, self).__init__(c)
        
        # Initialize manifold for hyperbolic space
        # Initialize layers for the line graph convolution
        # Initialize layers for the hyperbolic graph convolution
        self.manifold = getattr(manifolds, args.manifold)()
        dims, acts, _ = hyp_layers.get_dim_act_curv(args)
        hgc_layers = []
        for i in range(len(dims) - 1):
            c_in, c_out = self.c, self.c  # using the same curvature
            in_dim, out_dim = dims[i], dims[i + 1]
            act = acts[i]
            hgc_layers.append(
                hyp_layers.HyperbolicGraphConvolution(
        ...
        self.layers = nn.Sequential(*hgc_layers)
        self.line_conv = Line_conv_1(in_dim, out_dim)
        # Set encode_graph to True since we are using both graph structures
        self.encode_graph = True

    def encode(self, x, adj):
        """
        Forward pass for the dual-channel MHCNN_L.
        x: Node features
        adj: Adjacency matrix
        """
        # First channel: Line Graph Convolution
        line_graph_out = self.line_conv(x, adj)  # (N, out_dim) x 2708, 1434 

        # Second channel: Hyperbolic Graph Convolution
        x_tan = self.manifold.proj_tan0(x, self.c)
        x_hyp = self.manifold.expmap0(x_tan, c=self.c)
        x_hyp = self.manifold.proj(x_hyp, c=self.c)
        hyp_graph_out = super(MHCNN_L, self).encode(x_hyp, adj)  # (N, out_dim)

        # Combine the outputs from both channels (e.g., via summation, concatenation, etc.)
        output = line_graph_out + hyp_graph_out  # Element-wise summation (you can experiment with other combinations)

        return output


class MHCNN_D(Encoder):
    """
    Hypergraph and Derivative Graph dual-channel convolution network.
    """

    def __init__(self, c, args):
        super(MHCNN_D, self).__init__(c)
        
        # Initialize manifold for hyperbolic space
        self.manifold = getattr(manifolds, args.manifold)()

        # Initialize layers for the derivative graph convolution
        ...
        self.layers = nn.Sequential(*hgc_layers)
        self.derivate_conv = Derivate_conv_1(in_dim, out_dim)
        # Set encode_graph to True since we are using both graph structures
        self.encode_graph = True

    def encode(self, x, adj):
        """
        Forward pass for the dual-channel MHCNN_D.
        x: Node features
        adj: Adjacency matrix
        """
        # First channel: Derivative Graph Convolution
        derivate_graph_out = self.derivate_conv(x, adj)  # (N, out_dim)

        # Second channel: Hyperbolic Graph Convolution
        x_tan = self.manifold.proj_tan0(x, self.c)
        x_hyp = self.manifold.expmap0(x_tan, c=self.c)
        x_hyp = self.manifold.proj(x_hyp, c=self.c)
        hyp_graph_out = super(MHCNN_D, self).encode(x_hyp, adj)  # (N, out_dim)

        # Combine the outputs from both channels (e.g., via summation, concatenation, etc.)
        output = derivate_graph_out + hyp_graph_out  # Element-wise summation (you can experiment with other combinations)

        return output


class MHCNN_M(Encoder):
    """
    Hypergraph and Line Graph, derivate three-channel convolution network.
    """

    def __init__(self, c, args):
        super(MHCNN_M, self).__init__(c)
    ...
    def encode(self, x, adj):
        ...
        # First channel: Line Graph Convolution
        line_graph_out = self.line_conv(x, adj)  # (N, out_dim) x 2708, 1434
        derivate_graph_out = self.derivate_conv(x, adj)
        # Second channel: Hyperbolic Graph Convolution
        x_tan = self.manifold.proj_tan0(x, self.c)
        x_hyp = self.manifold.expmap0(x_tan, c=self.c)
        x_hyp = self.manifold.proj(x_hyp, c=self.c)
        hyp_graph_out = super(MHCNN_M, self).encode(x_hyp, adj)  # (N, out_dim)

        # Combine the outputs from both channels (e.g., via summation, concatenation, etc.)
        output = line_graph_out + derivate_graph_out + hyp_graph_out   # Element-wise summation (you can experiment with other combinations)
