...
from data.convert_to_hg import dhg_convert_hypernetx, other_hypergraph_convert_dhg, nxg_convert_g
from utils.dhg_hypergraph_utils import *
...
def load_data(args):
    ...
    data['adj_derv_norm'], _ = process(
            data['adj_derv'], data['features'], args.normalize_adj, args.normalize_feats
    )
    data['adj_line_norm'], _ = process(
            data['adj_line'], data['features'], args.normalize_adj, args.normalize_feats
    )
...
def load_data_lp(dataset, use_feats):
    if dataset in ['DrugBank', "DisGenet"]:
        adj, features, derv_adj, line_adj = load_single_bio_data(dataset, use_feats)
    elif dataset == 'biodata':
        adj, features, derv_adj, line_adj = load_bio_data(dataset, use_feats)
    else:
        raise FileNotFoundError(f"Dataset {dataset} is not supported.")

    # 构建线图邻接矩阵

    # 将超图邻接矩阵和线图邻接矩阵一起返回
    data = {'adj_train': adj, 'features': features, 'adj_derv': derv_adj, 'adj_line': line_adj}
    return data
...
def load_data_nc(dataset, use_feats):
    if dataset in ['CoauthorshipCora', 'CoauthorshipDBLP', 'CocitationCora', 'CocitationCiteseer', 'CocitationPubmed']:
        adj, features, labels, derv_adj, line_adj, idx_train, idx_val, idx_test = load_dhg_data(dataset, use_feats)
    elif dataset in ['CoauthorshipCombinate', 'CocitationCombinate']:
        adj, features, labels, derv_adj, line_adj, idx_train, idx_val, idx_test = load_multilayer_data(dataset, use_feats)
    else:
        raise FileNotFoundError('Dataset {} is not supported.'.format(dataset))
    labels = torch.LongTensor(labels)

    data = {'adj_train': adj, 'adj_derv': derv_adj, 'adj_line': line_adj, 'features': features, 'labels': labels, 'idx_train': idx_train, 'idx_val': idx_val, 'idx_test': idx_test}
    return data
...
def load_dhg_data(dataset_str, use_feats):
    ...
    # xgi_hg = dhg_convert_xgi(G)
    derivate_g = derivative_graph(G)
    hnx_hg = dhg_convert_hypernetx(G)
    line_g = hnx_hg.get_linegraph()
    derv_adj = nx.to_scipy_sparse_matrix(derivate_g)
    line_adj = nx.to_scipy_sparse_matrix(line_g)
    return sparse_adj, features, labels, derv_adj, line_adj, idx_train, idx_val, idx_test
...
def load_multilayer_data(dataset_str, use_feats):
    ...
    # xgi_hg = dhg_convert_xgi(mhg)
    derivate_g = derivative_graph(mhg)
    hnx_hg = dhg_convert_hypernetx(mhg)
    line_g = hnx_hg.get_linegraph()
    derv_adj = nx.to_scipy_sparse_matrix(derivate_g)
    line_adj = nx.to_scipy_sparse_matrix(line_g)
    return sparse_adj, features, labels, derv_adj, line_adj, idx_train, idx_val, idx_test
...
def load_single_bio_data(dataset_str, use_feats):
    ...
    derv_adj = nx.to_scipy_sparse_matrix(derivate_g)
    line_adj = nx.to_scipy_sparse_matrix(line_g)
    return sparse_adj, features, derv_adj, line_adj
