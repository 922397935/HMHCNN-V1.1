...
from models.mhcnn_base_models import NCModel, LPModel
from utils.train_utils import get_dir_name, format_metrics
from utils.dhg_mhcnn_data_utils import load_data
...
def train(args):
    ...
    data = load_data(args) # adj_train 2708, 2708 adj_derv 2388, 2388 adj_line 970, 970
    ...
    for epoch in range(args.epochs):
        ...
        model.train()
        optimizer.zero_grad()
        embeddings = model.encode(data['features'], data['adj_train_norm'], data['adj_line_norm'])
    ...
    if not best_test_metrics:
        model.eval()
        best_emb = model.encode(data['features'], data['adj_train_norm'], data['adj_line_norm'])
